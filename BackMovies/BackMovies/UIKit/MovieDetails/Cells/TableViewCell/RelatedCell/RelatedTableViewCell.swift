...
class RelatedTableViewCell: UITableViewCell {
    ...
    public func setUpCell(movieDetail: MovieDetailModel) {
        viewModel.setMovieDetail(movieDetail: movieDetail)
    }
...
extension RelatedTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell?.setUpCell(data: viewModel.getSimilarMovie(index: indexPath.row))
        return cell ?? UICollectionViewCell()
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        delegate?.navRelatedMovies(movieCellModel: viewModel.getSimilarMovie(index: indexPath.row))
    }
